...
namespace task1
{
  ...
  class Сonsultant : Client
  {
    ...
    /// Отчество
    /// </summary>
    public string PatronymicAsConsultant() { return this.patronymic;  }

    /// <summary>
    /// Серия и номер паспорта. Данные не доступны для просмотра.
    /// </summary>
    protected new string Passport()
    {
    ...
    {
      return $"{Surname}#{Name}#{PatronymicAsConsultant()}#{PhoneNumber}#{base.Passport}";
    }
    ...
    {
      return $"{Surname} {Name} {PatronymicAsConsultant()}. Teл. : {PhoneNumber}, Док-т : {Passport()}";
    }
